...
function normalizeSourceItem(it) {
  ...
  return {
    ...
    // Only ever populated by lib/ingest/config.js's buildNcertSourceSystem
    // (ncert_chapter docType) -- undefined/null for a newspaper_clipping
    // item, which normalizes the same way here since these are all optional.
    bookName: typeof it.bookName === "string" ? clampString(it.bookName, 200) : null,
    chapterName: typeof it.chapterName === "string" ? clampString(it.chapterName, 200) : null,
    ncertSubject: typeof it.ncertSubject === "string" ? clampString(it.ncertSubject, 100) : null,
    ncertClass: Number.isFinite(it.ncertClass) ? it.ncertClass : null,
    ...normalizeCommon(it),
  };
}
