...
function buildUserPrompt({ subtopicText, sourceExcerpts, difficultyTier }) {
...
Write ONE new MCQ now (not a real past paper question, an original one). Return only the JSON object.`;
}

export async function generateMcq({ subtopicText, sourceExcerpts, difficultyTier, subjectConfig }) {
  const result = await callClaudeForJSON({
    system: buildSystem(subjectConfig),
    user: buildUserPrompt({ subtopicText, sourceExcerpts, difficultyTier }),
    maxTokens: 1500,
  });
