...
const PRACTICE_A_SYSTEM = `You are writing critical-thinking material for a UPSC Civil Services Mains Law Optional aspirant's self-study on ONE syllabus subtopic. You will be given the core content (explanation, provisions, case law) already written for this subtopic — build on it, stay consistent with it, do not contradict it.
Ground every claim ONLY in the core content given to you or material you are highly confident is accurate. Do not invent citations.
...
Return ONLY a JSON object, no other text, in exactly this shape:
{
  ...
  "answerFramework": "<how to structure a Mains answer on THIS subtopic specifically — which of the provisions/cases above to lead with, how to sequence the argument, what a 'critically discuss' vs a fact-pattern version of this question would each need. HARD CAP: 120 words.>"
}
Provide exactly 3 perspectives. Every field has a hard word cap stated above — stop well before it, do not pad to fill it.`;

const PRACTICE_B_SYSTEM = `You are writing worked examples, self-check exercises, and memory aids for a UPSC Civil Services Mains Law Optional aspirant's self-study on ONE syllabus subtopic. You will be given the core content (explanation, provisions, case law) already written for this subtopic — build on it, stay consistent with it, do not contradict it.
Ground every claim ONLY in the core content given to you or material you are highly confident is accurate. Do not invent citations.
...
export async function generateLesson({ subtopicText, sourceExcerpts, caseAnchors }) {
  ...
  const [practiceRawA, practiceRawB] = await Promise.all([
    callClaudeForJSON({ system: PRACTICE_A_SYSTEM, user: practiceUser, maxTokens: 1200 }),
    callClaudeForJSON({ system: PRACTICE_B_SYSTEM, user: practiceUser, maxTokens: 1800 }),
  ]);
