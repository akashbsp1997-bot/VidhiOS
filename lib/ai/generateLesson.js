...
export async function generateLesson({ subtopicText, sourceExcerpts, caseAnchors, subjectConfig }) {
  ...
  const [practiceRawA, practiceRawB] = await Promise.all([
    callClaudeForJSON({ system: buildPracticeASystem(subjectConfig), user: practiceUser, maxTokens: 1200 }),
    callClaudeForJSON({ system: buildPracticeBSystem(subjectConfig), user: practiceUser, maxTokens: 1800 }),
  ]);
