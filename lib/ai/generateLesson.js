...
// Live failure (2026-07-20): the current free model (see lib/ai/client.js's
// MODEL) cut this core call off at finish_reason:"length" -- same failure
// already seen and fixed for lib/ingest/config.js's syllabus/pyq_paper
// calls. Raised these three budgets by roughly the same factor rather than
// re-tuning per-call from scratch; app/api/lesson/route.js's maxDuration
// was raised alongside this so the longer calls have room to actually
// finish instead of hitting a platform timeout instead.
export async function generateLesson({ subtopicText, sourceExcerpts, caseAnchors, subjectConfig }) {
  ...
  const [practiceRawA, practiceRawB] = await Promise.all([
    callClaudeForJSON({ system: buildPracticeASystem(subjectConfig), user: practiceUser, maxTokens: 3000 }),
    callClaudeForJSON({ system: buildPracticeBSystem(subjectConfig), user: practiceUser, maxTokens: 4000 }),
  ]);
