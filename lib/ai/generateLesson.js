...
{
  "teachContent": "<800-1200 word explanation: what the concept is, its basis, how it developed, the current position, and why it matters — written as connected prose in paragraphs separated by \\n\\n, not bullet fragments>",
  ${fieldBlocks}
}
...
// Live failures (2026-07-20): the current free model (see lib/ai/client.js's
// MODEL) cut BOTH the core call (at 3500) and, separately after that was
// raised, the practice-B call (at 4000) off at finish_reason:"length" --
// same underlying issue already seen for lib/ingest/config.js's
// syllabus/pyq_paper calls, just showing up call-site by call-site rather
// than everywhere at once. Unlike the ingest source-excerpt case, these
// three prompts already ask for tight, HARD-CAPPED word counts per field
// (see buildPracticeASystem/buildPracticeBSystem above) -- there's no
// oversized request to trim here, so raising the ceiling is the actual fix,
// not a stand-in for one. app/api/lesson/route.js's maxDuration was raised
// alongside the first round so the longer calls have room to finish instead
// of hitting a platform timeout.
export async function generateLesson({ subtopicText, sourceExcerpts, caseAnchors, subjectConfig }) {
  ...
  const core = normalizeCoreResult(coreRaw, subjectConfig.lessonSchema.structuredFields);
  ...
  const practice = normalizePracticeResult({ ...practiceRawA, ...practiceRawB });
