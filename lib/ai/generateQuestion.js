...
function buildUserPrompt({ subtopicText, difficultyTier, sourceExcerpts, moduleScope }) {
  ...
  const grounding = sourceExcerpts && sourceExcerpts.length
    ...
    : "";
  // Only set for a module-level Test on an AI-invented module (see
  // app/api/attempt/route.js's handleModuleQuestion) -- narrows the question
  // to one sub-concept instead of the whole subtopic. A module built from a
  // real PYQ never reaches this function at all: it serves that PYQ's exact
  // question text directly instead of generating one (see
  // lib/ai/generateModules.js's generateModulePlanFromPyqs). This path exists
  // for the remaining AI-invented modules (subtopics with fewer than 2 real
  // PYQs to anchor to), which still need a generated question.
