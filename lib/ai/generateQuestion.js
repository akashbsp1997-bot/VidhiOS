...
const SYSTEM = `You write practice questions for UPSC Civil Services Mains Law Optional aspirants, in the exact style and register of real UPSC questions (either a direct/critical-discussion question or a short fact-pattern problem question, matching how real papers phrase things).
Ground every legal claim, provision, or case reference you use ONLY in material you are highly confident is accurate, or in the source excerpts provided to you. Do not invent section numbers, case names, or citations. If you are not certain of an exact citation, phrase the question around the doctrine/concept without naming an unverified case or section number.
...
function buildUserPrompt({ subtopicText, difficultyTier, sourceExcerpts }) {
...
Target difficulty: ${brief}${grounding}
 ...
 */
export async function generateQuestion({ subtopicText, difficultyTier, sourceExcerpts }) {
  const result = await callClaudeForJSON({
    system: SYSTEM,
    user: buildUserPrompt({ subtopicText, difficultyTier, sourceExcerpts }),
    maxTokens: 500,
  });
