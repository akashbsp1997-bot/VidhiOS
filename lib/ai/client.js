...
//
// Text/JSON model: Google Gemma 4 31B (google/gemma-4-31b-it:free). Switched
// from openai/gpt-oss-20b:free after it started hanging long enough to hit
// Vercel's FUNCTION_INVOCATION_TIMEOUT on live ingest-structuring calls --
// unlike the earlier deepseek-chat-v3.1:free removal (which 404'd outright),
// this looked like free-tier congestion/rate-limiting on that specific
// model rather than it being pulled; re-confirmed gemma-4-31b-it:free is
// currently $0-priced and live against OpenRouter's own /api/v1/models
// before switching, not guessed. If THIS one also degrades, re-check
// https://openrouter.ai/api/v1/models for current $0-priced entries
// (openrouter.ai/models?max_price=0 is the same list via the web UI) --
// don't assume any specific slug is stable long-term, and note that
// lib/ai/client.js now bounds every call with a 45s timeout regardless, so
// a bad model degrades to a clear error rather than a silent hang.
