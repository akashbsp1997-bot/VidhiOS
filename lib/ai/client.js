...
//
// Text/JSON model: Gemini 2.5 Flash (google/gemini-2.5-flash). Switched off
...
// routing parameter needed, so this is a plain model-id swap. Priced (not
// $0) on paper, but billed against the linked Google account's own free
// tier, not an OpenRouter balance -- effectively free at this app's
// request volume. google/gemini-2.5-flash-image below is the same family,
// already unaffected by the old free-pool cap since it was never a :free
// model to begin with.
...
async function postToOpenRouter(url, apiKey, body) {
  for (let attempt = 0; attempt <= MAX_RATE_LIMIT_RETRIES; attempt++) {
    ...
    if (res.status === 429) {
      ...
      // that reads like a retry would help.
      ...
      throw new AIRequestError(`OpenRouter returned 429 (rate limited) after ${MAX_RATE_LIMIT_RETRIES} retries: ${bodyText.slice(0, 500)}`);
    }
