...
import { subjectUnlocks, subjects, subtopics, mastery, playerState } from "../../db/schema.js";
...
export async function checkLockdown(userId) {
  ...
  if (!state.lockedDown) return null;

  // A redeemed 'lockdown_grace' item (see lib/gamification/items.js) sets
  // playerState.lockdownGraceUntil -- while that's still in the future, the
  // student reads as not locked down regardless of the real missed-
  // checkpoint state above. Doesn't touch mastery or the checkpoint itself,
  // just suspends the app-wide restriction for a window.
  const [ps] = await db.select({ lockdownGraceUntil: playerState.lockdownGraceUntil }).from(playerState).where(eq(playerState.userId, userId));
  if (ps?.lockdownGraceUntil && new Date(ps.lockdownGraceUntil) > new Date()) return null;
